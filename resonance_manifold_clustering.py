...
class ResonanceManifoldClustering:
    ...
    def _build_adjacency(self, X):
        """
        ガウスカーネルで結合行列を構築し、弱い接続をゼロにする。

        修正点
        ------
        1. 対角成分（自己結合）を明示的に 0 にする。
           sin(0) = 0 なので計算上は影響がないが、
           「自分自身を引っ張る」という意味的な誤りを排除する。
        2. connectivity_threshold 未満の接続をゼロにする（スパース化）。
           微弱な結合によるクラスタ間の長距離同期を防ぐ。
        """
        dists = euclidean_distances(X, X)
        adj = np.exp(-dists ** 2 / (2 * self.sigma ** 2))
        np.fill_diagonal(adj, 0.0)
        adj[adj < self.connectivity_threshold] = 0.0  # スパース化
        return adj
